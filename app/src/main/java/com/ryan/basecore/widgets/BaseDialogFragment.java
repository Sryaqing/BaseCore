package com.ryan.basecore.widgets;

/**
 * @Author: Ryan
 * @Date: 2020/6/8 13:57
 * @Description: 避免内存泄漏, dialogFragment 实际上是对dialog的封装类，而dialog的dismiss和cancel状态是通过handler发送message去实现的，所以就算外部利用反射
 * 清空消息队列，这时handlerThread会处于阻塞状态，线程的堆内存的变量对象这时就无法释放，所以这里不实现官方的俩个状态接口，自己去定义的接口去监听，然后再手动销毁
 */
...
public class BaseDialogFragment extends Fragment {
    ...
    private DialogCancelListener mDialogCancleListener;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mDialogCancleListener = new DialogCancelListener(this);
    ...
    public static class DialogDismissListener implements DialogInterface.OnDismissListener {
        ...
        public DialogDismissListener(BaseDialogFragment leakDialogFragment) {
            this.leakDialogFragmentWeakReference = new WeakReference<>(leakDialogFragment);
        }

        @Override
        public void onDismiss(DialogInterface dialog) {
            BaseDialogFragment leakDialogFragment = leakDialogFragmentWeakReference.get();
    ...
    public static class DialogCancelListener implements DialogInterface.OnCancelListener {
        ...
        public DialogCancelListener(BaseDialogFragment leakDialogFragment) {
            this.leakDialogFragmentWeakReference = new WeakReference<>(leakDialogFragment);
        }
